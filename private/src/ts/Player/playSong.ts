...
function play(path: string) {
  ...
  $.ajax({
    ...
    success: (response) => {
      ...
      const data: string[] = [Album, Artist, AudioSource, Cover, Genre, Title];
 ...
 */
function playerInit(player: HTMLAudioElement, data: string[]) {
  // UI const
  const cover: JQuery<HTMLImageElement> = $("#player-cover");
  const title: JQuery<HTMLElement> = $("#player-title");
  const artist: JQuery<HTMLElement> = $("#player-artist");
 ...
 */
function updateProgress(player: HTMLAudioElement) {
  const progressBar: JQuery<HTMLProgressElement> = $(".progress-bar");
